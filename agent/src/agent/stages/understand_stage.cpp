...
WorkResult UnderstandStage::execute(AgentContext& ctx) {
    ...
    try {
        ...
        auto resp = llmComplete(ctx, {system_prompt, user_msg, /*json_mode=*/true, 0.1f, 1024});
        if (!resp.success) {
        ...
        if (auto* logger = ctx.logger())
            logger->blackboardWrite(ctx.config().agent_id, name, "agent:understanding", understanding);
